...
- Сабмит v6 (дистилляция): первый **Success на Check — Mean PR-AUC 0.3114** (2026-08-22, `jstnoname/distill_model_solution:v6`). Контрольный тест инференс-пути расхождений не дал; зазор к 0.5117 — шум малой выборки Check (~50 пар/категорию) и/или сдвиг распределения теста, сравнивать с платформенным скором baseline организаторов на той же фазе.
- Сабмит v7 (LoRA fine-tune): **Public Mean PR-AUC = 0.3999233091** (2026-08, `jstnoname/distill_model_solution:v7`). Парадигма: прямое LoRA-дообучение студента (`well-please/student_model`, ruBert-base) на human-метках (BCE, 0/1); учитель в лоссе не используется. Папка `models/fine-tuning`, абсолютные пути `/fine-tuning`.
...
- `pyproject.toml` — **единственный источник зависимостей и конфига**:
  - группы: core (pandas<3, polars, numpy, python-dotenv, mlflow==3.5.1, ipykernel, transformers>=5.15.0, torch==2.11.0, peft>=0.20.0, torchao>=0.18.0), dev (jupyterlab, matplotlib, scikit-learn), train (sentence-transformers, tqdm, joblib). requirements.txt в корне репо нет; у образа свой `models/*/requirements.txt` с рантайм-пинами (+cu128).
  - `[tool.ecup.data]` — data_repo и имена файлов HF-датасета; `[tool.ecup.model]` — параметры обучения: student_name=`well-please/student_model`, teacher_name/tokenizer_name=`well-please/teacher_model`, seed 69, student_epochs 2, student_batch 224, lr 1e-5, max_len 320, alpha 0.5, test_size 0.2, inference_batch 512. Читается ноутбуком через `utility.load()`, целиком логируется в MLflow.
  - `requires-python = ">=3.11"` (на Kaggle Python 3.11 — ужесточать нельзя).
- `utility/` — общий пакет для ноутбуков; в контейнер НЕ попадает (у `models/baseline/run.py` и `models/fine-tuning/run.py` свои инлайн-копии `product_text` и загрузки `CrossEncoder`).
...
- `models/fine-tuning/run.py` — аналогичный entry point v7, пути `/fine-tuning/{model,tokenizer}`.
...
- `metadata.json` = `{"image": "jstnoname/distill_model_solution:<vN>", "entry_point": "python -u run.py"}`; лежит в `models/baseline/` и `models/fine-tuning/`, копия едет в корне архива. Теги версионные (v1…vN), уже запушенные НЕ мутировать.
...
- `docker build -t jstnoname/distill_model_solution:v7 models/fine-tuning` — контекст сборки `models/fine-tuning/`.
...
- Стратифицированный сплит по категориям (`utility.sampling.train_test_split`) + length-bucketing (сортировка по длине перед батчингом) — текущая практика.
...
- `.dockerignore` в `models/baseline/` и `models/fine-tuning/` (исключает prep-артефакты) — страховка от утечки; в архив кладём только нужное для рантайма (`metadata.json`, `run.py`, `model/`, `tokenizer/`), без `.env`/служебных файлов.
...
- **Пути к весам — абсолютные `/baseline/{model,tokenizer}`** (v6) или **`/fine-tuning/{model,tokenizer}`** (v7), не от `__file__`: скрипт исполняется из чужого cwd.
...
- Токенизация инференса = обучению: парный режим `tokenizer(text1_list, text2_list)`, truncation longest_first, max_len 320; `padding=True` vs `'max_length'` на скоры НЕ влияет (attention_mask маскирует), но быстрее.
